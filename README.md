...
- [~] **Native indexer framework — sources Prowlarr can't reach, working *beside* Prowlarr, not instead of it.** Prowlarr only speaks Newznab/Torznab, so sites with no such API (HTML-scrape trackers, direct-download shadow libraries) are structurally out of its reach. A `type: native` indexer kind now sits alongside the synced `newznab`/`torznab` rows: each is a built-in Go implementation selected as the indexer's "type" (no URL), and all kinds feed the *same* `SearchAll` merge, release scoring, and grab pipeline — a native source is a peer indexer in the same list, not a bolt-on. Native rows are LibriNode-managed only and stay off the Readarr-facing surface (filtered out when the caller is Prowlarr), so Prowlarr never sees, syncs, or collides with them.
  ...
  - [ ] **Anna's Archive** (ebooks) + a general **`direct` protocol** (a LibriNode-side HTTP fetcher with mirror-list failover, so Libgen/open-book mirrors ride it too, then CDH imports like any grab). Still open — the direct-download seam; AudioBook Bay above already validates the framework end to end via the torrent path.
