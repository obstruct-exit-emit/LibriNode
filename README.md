...
— once you monitor the magazine (adds start unmonitored) — the automatic
search grabs newly published issues (capped at 3 per pass)
...
- [ ] **Live-verify the mock-tested integrations**: a real Prowlarr sync, a real torrent through qBittorrent, a real NZB through SABnzbd, and a real ComicVine key — end to end from search to organized file. The acquisition pipeline is only proven against mocks today; this is the biggest open risk
